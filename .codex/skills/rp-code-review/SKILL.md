...
| 5a 신원·권한 주체를 경계 바깥 파라미터로 수신 (E) | 보안 | 3 |
| 5a 그 외 판정표 적중 (소유권·권한 필드·열거 식별자·대량 처리 범위·역할 검사) | 보안 | 6 |

상한 6 이하는 최저 기준(≥7)에 자동 미달 — **별도 FAIL 경로 없음**.

**합성 규칙**: 동일 항목에 상한 복수 적중 → **가장 낮은 상한** 채택. 상한 + 감점 동시 → **감점 먼저, 그 뒤 상한 클램프** (`min(원점수 - 감점, 상한)`).

**5a 선행 판정 — 노출 등급**: 대상 **엔드포인트**를 **E**(외부 공개) / **I**(내부 전용) / **A**(운영자 전용) 로 먼저 확정. **미선언 시 E 기본값.** I·A 는 `TRUST-BOUNDARY` 증거 필수이며, 결손 또는 **증거(설정·코드 공통) 원문 열람·인용 불가 시 fail-closed → E 재판정**. 등급 정의·증거 요건·판정표 7행 → SSOT [`../harness-trust-boundary.md`](../harness-trust-boundary.md).
