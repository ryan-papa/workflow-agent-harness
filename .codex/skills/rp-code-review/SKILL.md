...
| 5a 신원·권한 주체를 경계 바깥 파라미터로 수신 + 검증 없음 (E 등급) | 보안 | 3 |
| 5a 소유권·역할 검사 누락 (E·A 등급) · 대량 처리 범위 미통제 (전 등급) | 보안 | 6 |

상한 6 이하는 최저 기준(≥7)에 자동 미달 — **별도 FAIL 경로 없음**.

**5a 선행 판정 — 노출 등급**: 대상 엔드포인트를 **E**(외부 공개) / **I**(메시·사설망 내부, 전송 계층 인가) / **A**(운영자 전용) 로 먼저 확정. **미선언 시 E 기본값.** I 는 `TRUST-BOUNDARY` 증거 3종(격리·전송 계층 인가·계약 명시) 필수이며 결손 시 선언 무효 → E 재판정. 등급별 해제 범위 → SSOT [`../harness-code-review.md`](../harness-code-review.md) 5a.
