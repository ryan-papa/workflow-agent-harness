...
| 5a 신원·권한 주체를 경계 바깥 파라미터로 수신 + 검증 없음 (E) | 보안 | 3 |
| 5a 소유권 결합 누락 (E·A·I 주체 동반) · 권한 필드·열거 식별자 (E·A) · 역할 검사 누락 (A) · 대량 처리 범위 미통제 (전 등급) | 보안 | 6 |
...
I·A 는 `TRUST-BOUNDARY` 증거 필수 — I 는 (1) 외부 도달 불가 (2) 허용 호출자 열거 + 기본 거부 (3) 주체 결정 책임이 호출측이라는 계약, **(4) 호출측 소유권 검증 지점**(안티패턴 2 조건부 해제 시에만), A 는 인증 설정 위치 + 역할 검사 위치. **수단은 스택 자유**(메시 정책·게이트웨이 허용목록·사설 서브넷·방화벽) — 판정은 사실이 설정 원문으로 확인되는가. **전송 계층 인증 단독은 인가 증거 불인정**(인증 ≠ 인가). 결손 또는 **리뷰어가 증거(설정·코드 공통) 내용을 직접 읽고 인용 불가 시 부재로 간주(fail-closed) → E 재판정.** 런타임 실효성은 어느 축도 검증하지 않는다. 등급별 해제 매트릭스·증거 상세 → SSOT [`../harness-code-review.md`](../harness-code-review.md) 5a.
