...
1. **⛔ Claude 엔지 리뷰는 반드시 Agent 툴의 서브에이전트로 실행** (`subagent_type=general-purpose`). 메인 에이전트의 셀프 채점 **금지**. 서브에이전트 프롬프트에는 (a) 읽을 PRD + 상위 CLAUDE.md·harness-db.md 등 참고 파일 경로, (b) 5항목 채점 기준, (c) 독립 판정 지시 포함
2. 서브에이전트가 평가 항목별 10점 만점 채점 + 기술 리스크 + 대안 반환
