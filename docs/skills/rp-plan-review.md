...
1. **⛔ Claude 기획 리뷰는 반드시 Agent 툴의 서브에이전트로 실행** (`subagent_type=general-purpose` 또는 지정 리뷰어). 메인 에이전트의 셀프 채점 **금지**. 서브에이전트 프롬프트에는 (a) 읽을 PRD 파일 경로 전체, (b) 9항목 채점 기준, (c) 독립 판정 지시(메인 컨텍스트 무시) 포함
2. 서브에이전트가 평가 항목별 10점 만점 채점 + 근거 + 개선 제안 반환
3. 메인 에이전트는 결과를 받아 판정만 수행
