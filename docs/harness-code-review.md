...
**원칙**: 신원·권한·등급·금액 등 **결과를 좌우하는 값은 서버가 결정**한다. 클라이언트 파라미터는 "무엇을 요청하는지"만 전달하고 "그럴 자격이 있는지"는 전달하지 않는다.

**적용 범위**: 스택 독립. 인증 컨텍스트 = 세션·JWT 클레임·서버 세션 스토어 등 **클라이언트가 변조 불가능한 출처**.

| 안티 패턴 | 예시 | 필수 방어 |
|-----------|------|----------|
| 신원 주체를 파라미터로 수신 | 회원탈퇴 대상을 `userId` 파라미터로만 받음 | 인증 컨텍스트에서 주체 추출. 타인 지정은 별도 관리자 엔드포인트 + 권한 검사 |
| 리소스 참조 시 소유권 미검증 | `GET /orders/{orderId}` 를 검증 없이 반환 | 조회 쿼리에 소유자 조건 결합 (`WHERE id = ? AND owner_id = ?`) — 조회 후 분기가 아닌 **쿼리 단계 결합** |
| 권한·등급·금액을 클라이언트가 전달 | 요청 바디의 `role`·`price`·`discount` 를 그대로 신뢰 | 서버 조회값 사용. 요청 바디의 해당 필드는 **무시 또는 거부** |
| 대량 처리 대상 범위를 클라이언트가 지정 | 필터·상한 없는 일괄 삭제·수정 API | 서버측 범위 상한 + 대상 전건 소유권·권한 검증 |
| 미검증 열거 가능 식별자 노출 | 연속 정수 PK 를 외부 API 에 그대로 노출 | 소유권 검증이 선행되면 허용. 검증 없는 열거 가능 ID 노출 금지 |

> **8점 질문:** 요청의 **모든 파라미터를 공격자가 임의 값으로 바꿔** 보내도 타인 리소스 접근·권한 상승·금액 조작이 불가능한가? → 답이 코드에 있으면 8+

**판정** (§5 루브릭 구간 매핑):

| 적중 조건 | 성격 | 보안 점수 상한 |
|-----------|------|:--------------:|
| 신원·권한 주체를 클라이언트 파라미터로 수신 + 서버측 검증 없음 | 인증 우회 가능 | **3** |
| 소유권 검증 누락 · 클라이언트 전달 권한 필드 신뢰 | 인가 누락 | **6** |

둘 다 통과 조건(최저 ≥7) 자동 미달. 리뷰어 재량 완화 불가.

**N/A 선언**: 인증이 불요한 공개 리소스만 다루는 변경은 N/A — 리뷰 본문에 "5a N/A: <이유>" 1줄 기록 후 skip.
...
| 5a 파라미터 위변조 (신원·권한 주체 수신) | 상한형 | 5 보안 | 인증 컨텍스트가 아닌 클라이언트 파라미터로 주체 결정 + 서버 검증 없음 → **최대 3점** |
| 5a 파라미터 위변조 (소유권·권한 필드 미검증) | 상한형 | 5 보안 | 소유권 검증 누락 또는 클라이언트 전달 권한 필드 신뢰 → **최대 6점** |
