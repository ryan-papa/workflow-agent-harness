...
**판정**: 신원·권한 주체를 클라이언트 파라미터로 받고 서버측 검증이 없으면 **전체 FAIL 고정**. 소유권 검증 누락·클라이언트 전달 권한 필드 신뢰는 **보안 최대 6점 상한**.
...
| 5a FAIL 조건 (신원·권한 주체 파라미터 수신) | 미적중 |
| 인프라 축 | BLOCK 0건 + 미해결 ASK 0건 |

최저 미달 시 평균 8.0 이상이어도 **미통과**. **1b·5a FAIL 조건 적중 시 점수와 무관하게 전체 FAIL (우회 불가)** — 재시도 카운트는 소비.
...
| 5a 파라미터 위변조 방지 | FAIL형 + 감점형 | 신원·권한 주체를 파라미터로 수신 + 서버 검증 없음 → **전체 FAIL 고정**. 소유권·권한 필드 검증 누락 → 보안 **최대 6점 상한** |
