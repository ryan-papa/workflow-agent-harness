...
| 3 | 등급별 판정표 7행 대조 → 적중 행의 **보안 상한** 적용 |

**보안 상한**: 신원·권한 주체를 경계 바깥 파라미터로 수신(E) → **최대 3점**. 그 외 적중(소유권 결합·권한 필드·열거 식별자·대량 처리 범위·역할 검사) → **최대 6점**. 상한 6 이하는 통과 조건(최저 ≥7) 자동 미달.
...
| 5a 파라미터 위변조 | 상한형 | 5 보안 | 판정표 7행 적중 시 **최대 3점 또는 6점** — 행·등급별 상세는 [`harness-trust-boundary.md`](harness-trust-boundary.md) §3 |
