...
**⛔ 하네스 절대 규칙 (예외 없음):**
- **[리뷰 단계 서브에이전트 필수]** `/rp-plan-review` · `/rp-eng-review` · `/rp-code-review` 의 Claude 리뷰는 **반드시 Agent 툴로 분리된 서브에이전트가 실행**한다. 메인 에이전트가 본인이 작성한 PRD·코드를 자체 채점하는 행위 **전면 금지**. 이해충돌·관성 편향 방지 목적. 서브에이전트는 메인 대화 컨텍스트 없이 대상 파일만 읽고 독립 판정해야 함. 생략·셀프 채점 발견 시 즉시 중단 + 재수행.
